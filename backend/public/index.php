...
$allowedOrigin  = $_ENV['FRONTEND_URL'] ?? getenv('FRONTEND_URL') ?: null;
$corsOrigin     = ($allowedOrigin && $requestOrigin === $allowedOrigin)
    ? $allowedOrigin
...
try {
    switch ($resource) {
        ...
        case 'leads':
            ...
            match (true) {
                $requestMethod === 'POST'   && $id !== null && $subAction === 'convert'        => $controller->convert($id),
                $requestMethod === 'POST'   && $id !== null && $subAction === 'promote_support' => $controller->promoteSupport($id),
                $requestMethod === 'GET'    && $id === null => $controller->index(),
                $requestMethod === 'GET'    && $id !== null => $controller->show($id),
                $requestMethod === 'POST'   && $id === null => $controller->store(),
                $requestMethod === 'PUT'    && $id !== null => $controller->update($id),
